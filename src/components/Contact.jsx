...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const access_key = '5e03e919-eb6d-4850-9b7c-e887f6d2272d';
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                ...
                className="w-full py-4 rounded-xl bg-accent text-background font-bold text-lg hover:bg-white transition-all duration-300 shadow-[0_0_20px_rgba(56,189,248,0.4)] hover:shadow-[0_0_25px_rgba(255,255,255,0.6)] disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {isSubmitting ? 'Sending...' : 'Send Message'}
              </button>

              {submitStatus === 'success' && (
                <div className="p-4 rounded-xl bg-green-500/20 border border-green-500/50 text-green-400 text-center animate-fade-in">
                  Message sent successfully! I'll get back to you soon.
                </div>
              )}
              {submitStatus === 'error' && (
                <div className="p-4 rounded-xl bg-red-500/20 border border-red-500/50 text-red-400 text-center animate-fade-in">
                  Oops! Something went wrong. Please check your Access Key or try again later.
                </div>
              )}
              {submitStatus === 'missing_key' && (
                <div className="p-4 rounded-xl bg-yellow-500/20 border border-yellow-500/50 text-yellow-500 text-center animate-fade-in font-medium">
                  Oops! You forgot to add your Web3Forms Access Key. Please open <span className="font-mono bg-black/20 px-2 py-1 rounded">Contact.jsx</span> and replace <span className="font-mono bg-black/20 px-2 py-1 rounded">'YOUR_WEB3FORMS_ACCESS_KEY'</span> with your actual key from https://web3forms.com/.
                </div>
              )}
            </form>
